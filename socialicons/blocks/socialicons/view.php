	...
	$hovercolor = $this->getConfig('hovercolor');
        ...
        if (is_array($this->getConfig('menu'))) {
            ...
            foreach($menu AS $id => $item){
				$path = 'modules/socialicons/lib/'.$item['icon'].'.svg';
				$svg = file_get_contents($path, FILE_USE_INCLUDE_PATH);	
                $url = $item['url'];
            ?>
					<?php  echo '<style>
						.socialicons #'.$item['icon'].' a {width: '.$size.'px;height: '.$size.'px;} 
						.socialicons #'.$item['icon'].' svg {width: '.$size.'px;height: '.$size.'px;fill: '.$color.'; )} 
						.socialicons #'.$item['icon'].' svg:hover {fill: '.$hovercolor.'; )} 
								</style>'; ?>
